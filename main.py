...
if not os.path.exists("rshq.json"):
    save_json("rshq.json", {"rshaq": "on", "coin": {}, "mode": {}, "id": {}, "shhn": {}, "tp": {}, "3dd": {}, "order": {}})
...
def process_update(update):
    ...
    # التحقق من قفل البوت (للأعضاء فقط)
    ...
    if text:
        if text.startswith('/start'):
            # معالجة روابط الدعوة ونقاط الإحالة
        ...
        # تنفيذ العمليات بحسب الـ mode المخزن للمستخدم
        rshq = load_json("rshq.json")
        ...
        if mode == "coins" and from_id == ADMIN:
            ...
            save_json("rshq.json", rshq)
            bot('sendMessage', {'chat_id': chat_id, 'text': f"أرسل الآن عدد النقاط لإضافتها أو خصمها (للخصم ضع علامة ناقص - مثل: -100) للحساب {text}:"})
            return
        ...
        # استقبال كود الهدية من المستخدم
        if mode == "hdia":
        ...
        # استقبال رابط حساب الرشق من المستخدم لبدء التنفيذ والطلب
        if mode == "to":
            tp = rshq.get('tp', {}).get(str(from_id))
            quantity = rshq.get('3dd', {}).get(str(from_id))
            if tp and quantity:
                # محاكاة إرسال طلب الرشق لموقع الـ API الخارجي بنجاح
                ...
                # إرسال إشعار لقناة البوت والأدمن بالتنفيذ والاكتمال
                bot('sendMessage', {'chat_id': ADMIN, 'text': f"🔔 طلب رشق جديد من العضو: {from_id}\nالعدد: {quantity}\nالرابط: {text}"})
            return

        # معالجة نظام إذاعة الرسائل من لوحة الأدمن
        if read_file("msg.php").strip() == "on" and from_id == ADMIN:
            all_u = file_lines("abbas.json")
            for uid in all_u:
                if uid.strip():
    ...
    if callback_query:
        ...
        # التجميع وكود الهدية للمستخدمين
        if data == 'collect':
        ...
        # معالجة طلب رشق الخدمات والشبكات الاجتماعية والكميات
        if data == 'order_rshq':
            bot('sendMessage', {
                'chat_id': chat_id,
        ...
        if data.startswith('runrshq|'):
            ...
            if user_coin >= int(price):
                ...
                rshq.setdefault("3dd", {})[str(from_id)] = 1000  # الكمية الافتراضية
                save_json("rshq.json", rshq)
        ...
        # التحكم بالإعدادات من لوحة الأدمن الإدارية
        if data == 'admin':
            if from_id == ADMIN:
                send_admin_panel(chat_id, message_id, from_id)
            return
        ...
        if data == 'lIllabbas': # إدارة نقاط الأعضاء
            if from_id == ADMIN:
                rshq = load_json("rshq.json")
...
# ========== دوال بناء وعرض القوائم الرسومية للمستخدم والأدمن ==========
...
def send_admin_panel(chat_id, message_id, from_id):
    text = "🛠 مرحباً بك يا أبا يوسف في لوحة التحكم الإدارية للبوت الخاص بك:"
    keyboard = [
